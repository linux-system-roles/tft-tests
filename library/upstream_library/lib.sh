...
lsrAnsibleGalaxy() {
    ...
    if [ -z "$ansible_module_path" ] || [ ! -d "$ansible_module_path/galaxy" ]; then
        rlLogError "Unable to find the Ansible Galaxy Python package"
        return 1
    fi
    galaxy_path=$ansible_module_path/galaxy

    if command -v python3 >/dev/null 2>&1; then
        patch_python=python3
    elif command -v python2 >/dev/null 2>&1; then
        patch_python=python2
    elif command -v python >/dev/null 2>&1; then
        patch_python=python
    else
        rlLogError "python3 or python2 is required to patch Ansible Galaxy"
        return 1
    fi

    "$patch_python" - "$galaxy_path" <<'PY'
...
marker = '# Patched by lsrAnsibleGalaxy for ANSIBLE_GALAXY_TIMEOUT'
import_pattern = re.compile(
    r'^(from ansible\.module_utils\.urls import .*\bopen_url\b.*)$',
    re.MULTILINE,
)
wrapper = """\
{marker}
...
for root, dirs, files in os.walk(galaxy_path):
    ...
    for name in files:
        ...
        with io.open(path, 'r', encoding='utf-8') as stream:
            source = stream.read()
        if marker in source or 'open_url(' not in source:
            continue

        match = import_pattern.search(source)
        if not match:
            raise SystemExit(
                'Found open_url call but no supported import in {0}'.format(path)
            )
        import_line = match.group(1)
        patched_import = re.sub(
            r'\bopen_url\b', 'open_url as _lsr_open_url', import_line, count=1
        )
        patched = source[:match.start()] + patched_import + '\n\n' + wrapper \
            + source[match.end():]
        backup = path + '.orig'
        ...
        print('Patched {0} (backup: {1})'.format(path, backup))
...
lsrInstallDependencies() {
    ...
    for req_file in $coll_req_file $coll_test_req_file; do
        ...
        if ! lsrAnsibleGalaxy collection install -p "$collection_path" -vv -r "$req_file"; then
